class Grid
{
  ...
  Grid(int _cols, int _rows, float _gutterSize, float _pageMargin)
  {
    cols = _cols;
    rows = _rows;
  ...
  float getW(float i){
    ...
    return modules[0][0].w*i +pageMargin;
    
  }
  
  float getSW(){
     return modules[0][0].w;
    
  }
    
  float getH(float i){
    
    return modules[0][0].h*i+pageMargin;
    
  }
  
  
    float getSH(){
     return modules[0][0].h;
