...
void setup() {
 ...
 polygon  = new Polygon2D(polygonPoints);
 ArrayList<Vec2D> converted = new ArrayList<Vec2D>();
 for(int i=0; i<outerPoints.size();i++){
  PVector p = outerPoints.get(i);
  converted.add(new Vec2D( p.x,p.y));
 }
 pentagon = new Polygon2D(converted);
...
void draw() {
  ...
  for (Polygon2D pp : breakParts) {
    ...
    gfx.polygon2D(pp);
//  to check mouseposition
  //ellipse(p.x, p.y, 50, 50);
  }
 if(!br){
 beginShape();
 for(PVector p : outerPoints){
...
//    for (;;) {
  ...
  p = new Vec2D(mouseX-center.x,-(mouseY-center.y));
  println(p);
  fill(255, 0, 0);
  
  
  //outerPoints.add(new PVector(p.x,p.y));
  //add 5 more points based on first point.

    for(;;){
      Vec2D ppp = new Vec2D(p.x+random(-50, 50), p.y+random(-50, 50));
      if(pentagon.containsPoint(ppp))  outerPoints.add(new PVector(ppp.x,ppp.y));
    if(outerPoints.size()>20) break;  
  }
