...
ArrayList centerTriangles = new ArrayList();

ArrayList centerPolygon = new ArrayList();
...
void setup() {
  ...
  ArrayList tempCenter = new ArrayList();

  for (PVector p : sPoints) {

    tempCenter.add(new Vec2D(p.x, p.y));
  }

  centerPolygon.add(new Polygon2D(tempCenter));
...
//  beginShape();
  ...
  vertex(spj.x, spj.y);
  endShape();
    Vec2D cv0 = new Vec2D(spi.x, spi.y);
    Vec2D cv1 = new Vec2D(pj.x, pj.y);
    Vec2D cv2 = new Vec2D(spj.x, spj.y);
    ...
    centerPolygon.add(new Polygon2D(ctemp));
  }
  ...
  for (int i=0; i<polygons.size(); i++) {
    ...
    for (;;) {
      ...
      if (p.containsPoint(pv)) {
        ...
        centerPoints.add(tempP);
        if (pointsList.get(i).size()>10) break;
      }
    }
  }
  ...
  println(centerPolygon.size());
  
   for (int i=0; i<centerPolygon.size(); i++) {
     
    Polygon2D p = (Polygon2D)centerPolygon.get(i);
   // pointsList.get(i).add(sPoints.get(i));
  ...
  centerTriangles  = Triangulate.triangulate(centerPoints);
...
//  endShape();
  ...
  //    fill(random(100, 205));
    noFill();
    Triangle t = (Triangle)centerTriangles.get(i);
    vertex(t.p1.x, t.p1.y);
    vertex(t.p2.x, t.p2.y);
    vertex(t.p3.x, t.p3.y);
  }
